...
if __name__ == "__main__":
    ...
    src = spark.read.jdbc(pg_url(), "prod.branches", properties=pg_props())
    print(f"Read {src.count()} rows from prod.branches")
    ...
    fact = (
        ...
        .select(
            ...
            col("city_id").cast("string").alias("city_id"),
            col("branch_start_date").cast("date").alias("branch_start_date"),
    ...
    (
        ...
        .jdbc(ch_url(), "branches", properties=ch_props())
    )

    # Sanity read-back
    back = spark.read.jdbc(ch_url(), "branches", properties=ch_props())
    back.orderBy("branch_start_date").show(5, truncate=False)

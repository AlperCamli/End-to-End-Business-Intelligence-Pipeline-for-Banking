...
if __name__ == "__main__":
    ...
    new_src = src.groupBy("src.segment").pivot("src.is_essential").count().alias("src")

    new_src = new_src.join(count_src, new_src.segment == count_src.segment, "left")

    new_src = new_src.select(
        col("src.segment"), 
        (col("true")/col("count_src.count")).cast("decimal(18,2)").alias("Essential"),
        (col("false")/col("count_src.count")).cast("decimal(18,2)").alias("Non_Essential")
    ...
    (
        ...
        .jdbc(ch_url_Target(), "is_essential_avg_dm", properties=ch_props())
    )

    # Sanity read-back
    back = spark.read.jdbc(ch_url_Target(), "is_essential_avg_dm", properties=ch_props())

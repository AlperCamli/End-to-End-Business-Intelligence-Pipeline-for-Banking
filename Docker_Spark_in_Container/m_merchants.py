...
if __name__ == "__main__":
    ...
    src = spark.read.jdbc(pg_url(), "prod.merchants", properties=pg_props())
    print(f"Read {src.count()} rows from prod.merchants")

    fact = (
        src
        .select(
            col("merchant_id").cast("string").alias("merchant_id"),
            col("merchant_name").cast("string").alias("merchant_name"),
    ...
    (
        ...
        .jdbc(ch_url(), "merchants", properties=ch_props())
    )

    # Sanity read-back
    back = spark.read.jdbc(ch_url(), "merchants", properties=ch_props())
    back.orderBy("merchant_id").show(5, truncate=False)

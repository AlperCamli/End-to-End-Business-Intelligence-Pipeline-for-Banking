...
if __name__ == "__main__":
    ...
    src = spark.read.jdbc(pg_url(), "prod.accounts", properties=pg_props())
    print(f"Read {src.count()} rows from prod.accounts")

    fact = (
        src
        .select(
            col("account_number").cast("string").alias("account_number"),
            col("customer_id").cast("string").alias("customer_id"),
            col("initial_balance").cast("decimal(18,2)").alias("initial_balance"),
            col("overdraft_limit").cast("decimal(18,2)").alias("overdraft_limit"),
            col("branch_id").cast("string").alias("branch_id"),
            col("account_open_date").cast("date").alias("account_open_date"),
    ...
    (
        ...
        .jdbc(ch_url(), "accounts", properties=ch_props())
    )

    # Sanity read-back
    back = spark.read.jdbc(ch_url(), "accounts", properties=ch_props())
    back.orderBy("account_number").show(5, truncate=False)

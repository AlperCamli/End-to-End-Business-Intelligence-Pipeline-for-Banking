...
if __name__ == "__main__":
    ...
    src = spark.read.jdbc(pg_url(), "prod.cities", properties=pg_props())
    print(f"Read {src.count()} rows from prod.cities")

    fact = (
        src
        .select(
            col("city_id").cast("string").alias("city_id"),
            col("city_name").cast("string").alias("city_name"),
        )
    )
    ...
    (
        ...
        .jdbc(ch_url(), "cities", properties=ch_props())
    )

    # Sanity read-back
    back = spark.read.jdbc(ch_url(), "cities", properties=ch_props())
    back.orderBy("city_id").show(5, truncate=False)

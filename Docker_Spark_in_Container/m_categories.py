...
if __name__ == "__main__":
    ...
    src = spark.read.jdbc(pg_url(), "prod.categories", properties=pg_props())
    print(f"Read {src.count()} rows from prod.categories")
    ...
    fact = (
        ...
        .select(
            ...
            col("is_essential").cast("boolean").alias("is_essential"),
    ...
    (
        ...
        .jdbc(ch_url(), "categories", properties=ch_props())
    )

    # Sanity read-back
    back = spark.read.jdbc(ch_url(), "categories", properties=ch_props())
    back.orderBy("category_id").show(5, truncate=False)

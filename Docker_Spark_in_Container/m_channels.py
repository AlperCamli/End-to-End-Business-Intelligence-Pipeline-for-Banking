...
if __name__ == "__main__":
    ...
    src = spark.read.jdbc(pg_url(), "prod.channels", properties=pg_props())
    print(f"Read {src.count()} rows from prod.channels")

    fact = (
        src
        .select(
            col("channel_id").cast("string").alias("channel_id"),
            col("channel_name").cast("string").alias("channel_name"),
    ...
    (
        ...
        .jdbc(ch_url(), "channels", properties=ch_props())
    )

    # Sanity read-back
    back = spark.read.jdbc(ch_url(), "channels", properties=ch_props())
    back.orderBy("channel_id").show(5, truncate=False)

...
if __name__ == "__main__":
    ...
    # Keep date as STRING (YYYY-MM-DD). Ignore time completely.
    fact = (
        src
        .withColumn("transaction_date", date_format(col("transaction_date"), "yyyy-MM-dd"))  # -> String
        .withColumn("amount", col("transaction_amount").cast("double"))
        .withColumn("type", col("transaction_type").cast("string"))
        .select(
    ...
    # Sanity read-back
    back = spark.read.jdbc(ch_url(), "transactions", properties=ch_props())
    back.orderBy("account_number", "transaction_date").show(5, truncate=False)
